...
var QuantitySelector = class extends CustomHTMLElement {
  connectedCallback() {
    ...
    this.delegate.on("click", "button:first-child", this.stepDown.bind(this));
    this.delegate.on("click", "button:last-child", this.stepUp.bind(this));
  }
  stepUp() {
...
var Timeline = class extends HTMLElement {
  ...
  select(index, animate = true) {
    ...
    if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
      ...
      const scrollPosition = isRTL ? -(this.listItemElements[0].clientWidth * index) : this.listItemElements[0].clientWidth * index;
...
var DesktopNavigation = class extends CustomHTMLElement {
  connectedCallback() {
    ...
    // UNPATCHED: dropdowns are intentionally click-driven. Hover must never open or switch menus.
  ...
  let openingTimeout = setTimeout(() => {
    ...
    if (dropdown.classList.contains("mega-menu") && !MediaFeatures.prefersReducedMotion()) {
      const items = Array.from(
        dropdown.querySelectorAll(".mega-menu__column, .mega-menu__image-push")
      );
...
var StoreHeader = class extends CustomHTMLElement {
  ...
  _checkTransparentHeader(event) {
    if (this.matches(":scope:hover")) {
      return this.classList.remove("header--transparent");
    }
...
onRerender_fn = function(event) {
  ...
  if (!this.hasAttribute("allow-partial-rerender") || event.detail.productChange) {
    ...
    const blockTypes = ["meta", "payment-terms", "variant-picker", "volume-pricing", "quantity-selector", "inventory", "buy-buttons", "pickup-availability", "liquid"];
...
var ProductImageZoom = class extends OpenableElement {
  ...
  async _openPhotoSwipe() {
    ...
    this.photoSwipeInstance.listen("close", () => {
      this.open = false;
      this.photoSwipeInstance.ui.destroy();
    });
    this.photoSwipeInstance.listen("afterChange", () => {
      setTimeout(() => {
        this.photoSwipeInstance.currItem.container?.querySelector("img")?.setAttribute("alt", this.photoSwipeInstance.currItem.alt);
      }, 2e3);
    });
...
var QuickBuyDrawer = class extends DrawerContent {
  connectedCallback() {
    super.connectedCallback();
    this.addEventListener("openable-element:close", this._onClose.bind(this));
  }
  async _load() {
    if (!this.requiresLoading) {
      return;
